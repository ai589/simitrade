...
robocopy "%~dp0." "%WORK%" /MIR /XD .git .vercel __pycache__ .claude logs output scratch /XF px5y_cache.json px10y_cache.json px10y_cache.meta.json earnings_cache.json universe.json tiger_config.json orders_sent.json refresh_log.txt .env.local CLAUDE.md /NFL /NDL /NJH
if errorlevel 8 (
