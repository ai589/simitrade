...
robocopy "%~dp0." "%WORK%" /MIR /XD .git .vercel __pycache__ .claude logs /XF px5y_cache.json px10y_cache.json earnings_cache.json refresh_log.txt publish_log.txt .env.local CLAUDE.md /NFL /NDL /NJH >> logs\publish_log.txt 2>&1
if errorlevel 8 (
